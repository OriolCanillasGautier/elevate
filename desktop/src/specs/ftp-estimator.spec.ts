...
import { Activity, ActivityFlag, ActivityStats, PowerStats, Peak, HeartRateStats } from "@elevate/shared/models/sync/activity.model";
...
describe("FtpEstimator", () => {
  ...
  describe("estimateFromPeaks", () => {
    ...
    it("should filter out activities with abnormal power flags", () => {
      ...
      activities.push(
        createMockActivity("clean_1", date.toISOString(), 250, generateCPPeaks(250, 20000), 270)
      );
      ...
      for (let i = 0; i < 40; i++) {
        ...
        activities.push(
          createMockActivity(
            ...
            avgPower,
            peaks,
            Math.round(np + 18000 / 1200),
            null,
            { weighted: Math.round(np), variabilityIndex: 1.05, movingTime: 3600, elapsedTime: 3900 }
          )
        );
      }
      ...
      for (let i = 0; i < 40; i++) {
        ...
        activities.push(
          createMockActivity(
            ...
            175,
            peaks,
            Math.round(250 + 20000 / 1200),
            null,
            { weighted: 200, variabilityIndex: 1.05, movingTime: 3600, elapsedTime: 3900 }
          )
        );
      }

      const manualFtps = new Map<string, number>();
      const manualDate = new Date(REF_DATE.getTime() - 30 * 24 * 60 * 60 * 1000)
        .toISOString()
        .split("T")[0];
    ...
    it("should exclude trainer rides when excludeTrainer is true", () => {
      ...
      for (let i = 0; i < 10; i++) {
        ...
        const act = createMockActivity(
          ...
          undefined,
          null,
          { weighted: 230, variabilityIndex: 1.05, movingTime: 3600, elapsedTime: 3900 }
        );
        act.trainer = true;
        activities.push(act);
      }
  ...
  describe("estimateFromActivity", () => {
    ...
    it("should classify medium rides with moderate VI as tempo", () => {
      const activity = createDetailedActivity({
        movingTime: 70 * 60,
        elapsedTime: 72 * 60,
        variabilityIndex: 1.10 // Between 1.08 and 1.12 → tempo
      });
      const result = FtpEstimator.estimateFromActivity(activity, ATHLETE_WEIGHT);
