...
export class FtpEstimator {
  ...
  private static classifyRideIntensity(activity: Activity): "endurance" | "tempo" | "threshold" {
    ...
    if (vi > 1.20 && durationMinutes > 60) return "endurance";
   ...
   */
  public static estimateFromActivity(
    activity: Activity,
    athleteWeight: number
  ): ActivityFtpAnalysis | null {
    ...
    if (best20min > 0) {
      ...
      if (rideIntensity === "threshold") {
        ...
        trust = peak20toNp > 1.20 ? "medium" : "low";
        rationale = peak20toNp > 1.20
        ...
        trust = "low";
        rationale = "This was an endurance ride — the best 20-min segment was well below your capacity. This is a lower bound, not an FTP estimate.";
      }
    ...
    if (best60min > 0) {
      ...
      if (rideIntensity === "threshold") {
        trust = "high";
        rationale = "FTP is the max power you can sustain for 1 hour. This hard ride likely produced a near-maximal 60-min effort.";
      } else if (rideIntensity === "tempo") {
        trust = "low";
        rationale = "This ride's pace was below threshold. The best 60-min average is a lower bound — your true 1-hour max is higher.";
      } else {
        trust = "low";
        rationale = "This was an endurance ride. The best 60-min average is well below what you could sustain at max effort. This is a floor, not your FTP.";
      }
    ...
    if (np > 0 && rideDuration >= FtpEstimator.MIN_NP_RIDE_DURATION) {
      ...
      if (rideIntensity === "threshold") {
        ...
        trust = "medium";
        rationale = `Tempo ride — NP adjusted by VI (×${_.round(effectiveVI, 2)}) to estimate FTP. Moderate confidence since the ride was below threshold.`;
      } else {
        trust = "medium";
        rationale = `Endurance ride — NP adjusted by VI (×${_.round(effectiveVI, 2)}) to account for sub-threshold pacing. Most accurate indicator for easy rides.`;
      }
    ...
    if (cpResult) {
      ...
      if (rideIntensity === "endurance") {
        ...
        trust = "low";
        rationale = `The CP model fits the data well (R²=${_.round(cpResult.params.rSquared, 3)}), but this was an endurance ride — all efforts were below capacity. This is a lower bound.`;
      } else if (rideIntensity === "threshold" && cpResult.params.rSquared >= 0.97) {
        trust = "high";
        rationale = `Strong curve fit (R²=${_.round(cpResult.params.rSquared, 3)}) on a hard ride with diverse efforts.`;
      } else if (cpResult.params.rSquared >= FtpEstimator.MIN_SINGLE_RIDE_R_SQUARED) {
        trust = rideIntensity === "threshold" ? "medium" : "low";
        rationale = `Good curve fit (R²=${_.round(cpResult.params.rSquared, 3)}). ${rideIntensity === "tempo" ? "Tempo ride — estimate may be slightly low." : "More varied, hard efforts would improve reliability."}`;
      } else {
        trust = "low";
        rationale = `Moderate curve fit (R²=${_.round(cpResult.params.rSquared, 3)}). The data may not represent your true capacity.`;
      }
      ...
      indicators.push({
        ...
        trust,
        rationale,
        detail: `CP: ${_.round(cpResult.params.cp, 0)}W · W': ${_.round(cpResult.params.wPrime / 1000, 1)} kJ · R²: ${_.round(cpResult.params.rSquared, 3)}`
      });
    }
    ...
    const bestIndicator = indicators.length > 0
      ? _.maxBy(indicators, i => trustRank[i.trust] * 1000 + i.ftp) || null
      : null;
  ...
  private static trySingleRideCpModel(
    ...
    const activityDate = typeof activity.startTime === "string"
      ? activity.startTime
    ...
    if (best20min > 0 && !pdPoints.find(p => p.durationSeconds === 1200)) {
      pdPoints.push({
        durationSeconds: 1200, power: best20min,
        activityDate, activityId: activity.id, qualityScore: 0.85
      });
    }
  ...
  public static async estimate(
    ...
    for (const activity of cyclingActivities) {
      ...
      try {
        ...
        if (streams?.watts?.length > 0 && streams?.time?.length > 0) {
          const streamPoints = FtpEstimator.extractPointsFromStreams(
            streams,
            activity,
            athleteWeight
          );
    ...
    // Build date range
    const activityDates = cyclingActivities
      .filter(a => contributingActivityIds.includes(a.id))
      .map(a => a.startTime);
  ...
  public static computeTrend(
    ...
    for (const activity of cyclingActivities) {
      ...
      let weight: number;
      switch (intensity) {
        case "threshold": weight = 1.0; break;
        case "tempo": weight = 0.7; break;
        case "endurance": weight = 0.5; break;
      }
    ...
    const getDecayRate = (fromDate: string, toDate: string): number => {
      ...
      if (ctlByDate) {
        ...
        if (ctlFrom != null && ctlTo != null && ctlFrom > 0) {
          ...
          if (ctlDrop > 0.05) {
            rate *= 1.0 + Math.min(ctlDrop / 0.20, 1.0);
          }
        }
      }
      return rate;
    ...
    while (currentMs <= endMs) {
      ...
      const confidenceLabel: "high" | "moderate" | "low" | "insufficient" =
        ...
        confidence >= 20 ? "low" : "insufficient";
    ...
    if (trendPoints.length === 0 || trendPoints[trendPoints.length - 1].date !== finalDateStr) {
      ...
      const ridesInWindow = rideEstimates.filter(
        r => r.dateMs >= endMs - lookbackMs && r.dateMs <= endMs
      ).length;
   ...
   */
  private static filterCyclingActivities(
    activities: Activity[],
    windowDays: number,
    referenceDate: Date
  ): Activity[] {
  ...
  private static computeConfidence(
    ...
    if (modelFit < 0.85) {
      tips.push("Your power data points don't follow a smooth power-duration curve. More consistent efforts will help.");
    }
