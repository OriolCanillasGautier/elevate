...
export class EffortDetector {
  ...
  private static readonly RELATIVE_POWER_THRESHOLD = 0.4;
  ...
  private static readonly MIN_PEDALING_RATIO = 0.8;
  ...
  private static detectSustainedSegments(
    ...
    for (let i = 0; i < powerArray.length; i++) {
      ...
      if (isPedaling && segmentStart === -1) {
        ...
        if (duration >= EffortDetector.MIN_EFFORT_DURATION && duration <= EffortDetector.MAX_EFFORT_DURATION) {
    ...
    if (heartRateArray && heartRateArray.length > endIdx) {
      ...
      if (windowHR.length > 10) {
        ...
        if (avgPower > 0 && avgHeartRate > 60) {
          ...
          hrPowerCoupled = EffortDetector.checkHrPowerCoupling(heartRateArray.slice(startIdx, endIdx + 1), windowPower);
        }
      }
    }
   ...
   */
  private static checkHrPowerCoupling(hrValues: number[], powerValues: number[]): boolean {
  ...
  private static computeQualityScore(
    ...
    if (variabilityIndex <= 1.02) {
      ...
      viScore = 0.95;
    } else if (variabilityIndex <= 1.1) {
      viScore = 0.85;
    } else if (variabilityIndex <= 1.15) {
      viScore = 0.7;
    } else if (variabilityIndex <= 1.2) {
      viScore = 0.5;
